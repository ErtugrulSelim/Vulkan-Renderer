...
void App::build_glfw_window(int width, int height, bool debugMode) {
	...
	if (window = glfwCreateWindow(width, height, "Render Engine", nullptr, nullptr)) {
		if (debugMode) {
			std::cout << "Successfully made a glfw window called \"Render Engine\", width: " << width << ", height: " << height << '\n';
		}
	}
	else {
		if (debugMode) {
